...
from torchvision.models import vgg16, resnet101, resnet152
import wandb
...
"""
class ClassifierNCNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.name = "Classifier_n_CNN"

        # ResNet
        self.ResNet = resnet152(pretrained=True)
        # num_ftrs = self.ResNet.fc.in_features
        # self.ResNet.fc = nn.Linear(num_ftrs, 40)
...
class ClassifierNCNNTrainer():
    def __init__(self, args) -> None:
        ...
        self.model = ClassifierNCNN()

        self.lossfn = LossFN()
        self.evaluater = ClassifierNCNNEvaluater(self.data_loader_val)
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            for step, sample_batched in enumerate(tqdm(self.data_loader)):
                ...
                gld = [anno[item].tolist() for item in anno]
    ...
    def evaluate(self, model):
        ...
        with torch.no_grad():
            for step, sample_batched in enumerate(tqdm(self.data_loader)):
                ...
                correct_pred = 0
                for i in range(gld.shape[0]):
                    if torch.abs(gld[i]-outputs[i])<0.5:
                        correct_pred += 1
                total_acc += correct_pred / gld.shape[0]
